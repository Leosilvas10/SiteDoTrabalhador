...
async function fetchRealJobsFromBrazil() {
  try {
    ...
    console.log('🔄 Buscando vagas de múltiplas fontes...');

    // ESTRATÉGIA ROBUSTA: Múltiplas fontes em paralelo
    const promises = [];

    // 1. Indeed Brasil (Scraping otimizado)
    promises.push(
      scrapeIndeedBrasil().then(jobs => {
        if (jobs.length > 0) {
          allJobs.push(...jobs);
          usedSources.push('Indeed Brasil');
          console.log(`✅ Indeed: ${jobs.length} vagas`);
        }
        return jobs;
      }).catch(error => {
    ...
    // 2. Vagas.com (Scraping otimizado)
    promises.push(
      scrapeVagasCom().then(jobs => {
        if (jobs.length > 0) {
          allJobs.push(...jobs);
          usedSources.push('Vagas.com');
          console.log(`✅ Vagas.com: ${jobs.length} vagas`);
        }
        return jobs;
      }).catch(error => {
    ...
    // 3. API VagasBR (GitHub)
    promises.push(
      fetchVagasBRAPI().then(jobs => {
        if (jobs.length > 0) {
          allJobs.push(...jobs);
          usedSources.push('VagasBR GitHub');
          console.log(`✅ VagasBR: ${jobs.length} vagas`);
        }
        return jobs;
      }).catch(error => {
    ...
    // 4. API Emprega Brasil
    promises.push(
      fetchEmpregaBrasilAPI().then(jobs => {
        if (jobs.length > 0) {
          allJobs.push(...jobs);
          usedSources.push('Emprega Brasil');
          console.log(`✅ Emprega Brasil: ${jobs.length} vagas`);
        }
        return jobs;
      }).catch(error => {
    ...
    // 5. API Trabalha Brasil
    promises.push(
      fetchTrabalhaBrasilAPI().then(jobs => {
        if (jobs.length > 0) {
          allJobs.push(...jobs);
          usedSources.push('Trabalha Brasil');
          console.log(`✅ Trabalha Brasil: ${jobs.length} vagas`);
        }
        return jobs;
      }).catch(error => {
        console.log('⚠️ Trabalha Brasil erro:', error.message);
        return [];
      })
    );

    // 6. Nova API - InfoJobs Brasil
    promises.push(
      fetchInfoJobsBrasil().then(jobs => {
        if (jobs.length > 0) {
          allJobs.push(...jobs);
          usedSources.push('InfoJobs Brasil');
          console.log(`✅ InfoJobs: ${jobs.length} vagas`);
        }
        return jobs;
      }).catch(error => {
        console.log('⚠️ InfoJobs indisponível:', error.message);
        return [];
      })
    );

    // 7. Nova API - Catho Simulado
    promises.push(
      fetchCathoSimulado().then(jobs => {
        if (jobs.length > 0) {
          allJobs.push(...jobs);
          usedSources.push('Catho Simulado');
          console.log(`✅ Catho: ${jobs.length} vagas`);
        }
        return jobs;
      }).catch(error => {
    ...
    // Garantir mínimo de 300 vagas
    if (allJobs.length < jobCache.minJobs) {
      console.log(`📊 Complementando: ${allJobs.length} < ${jobCache.minJobs}`);
      const additionalJobs = generateBrazilianMarketJobs(jobCache.minJobs - allJobs.length);
      allJobs.push(...additionalJobs);
      usedSources.push('Gerador Brasileiro');
    }

    // Processar e otimizar dados
    const processedJobs = processJobData(allJobs, jobCache.maxJobs);
    ...
    const emergencyJobs = generateBrazilianMarketJobs(jobCache.minJobs);
...
async function fetchJobsForHome() {
  try {
    ...
    if (homeJobsCache.lastUpdate && (Date.now() - homeJobsCache.lastUpdate) < homeJobsCache.ttl) {
      ...
      return homeJobsCache.data.slice(0, 20); // Mostrar apenas 20 na home
    }
    ...
    if (allJobsResult.success && allJobsResult.data.length > 0) {
      ...
      return homeJobs.slice(0, 20); // Retornar apenas 20 para a home
    }
...
function processJobData(jobs, maxJobs = 500) {
  ...
  console.log(`✅ ${sortedJobs.length} vagas únicas processadas`);
  return sortedJobs;
}
