...
const $_SESSION = {
  add: (data) =>
    memcached.add(data.id, JSON.stringify(data), 60 * 60, (err) => {
      /* stuff */
    }),
  get: (id) =>
    new Promise((resolve, reject) =>
      memcached.gets(id, (err, data) => {
        if (err || !data) {
          reject(err);
            ...
            req.token = JWT.sign(
              {
                id: decoded.id,
                name: decoded.name,
                user: session[req.body.user].user,
              },
              session[req.body.user].aToken,
  ...
  await session[type]
    ...
    .then((data) => {
      ...
      req.access_token = JWT.sign(
        { id: data.id, name: data.name, user: session[type].user },
        session[type].aToken,
